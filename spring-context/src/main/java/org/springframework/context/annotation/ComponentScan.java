...
public @interface ComponentScan {
    // @ComponentScan 的功能是自动扫描并加载符合条件的组件(如：@Component、@Repository等)或 Bean 定义，最终将这些 Bean 定义加载到 IoC 容器中。
    // 可以通过 basePackages 等属性来细粒度定制 @ComponentScan 自动扫描的范围；如果不指定，则 Spring 框架默认从声明 @ComponentScan 所在类的 package 进行扫描。

    /**
     * Alias for {@link #basePackages}.
     * <p>
     * Allows for more concise annotation declarations if no other attributes are needed &mdash;
